...
int n;
// Function definition of change password facility
void change_password(){
    char s[30],p[30];
    int q,k=0;
    int chku,chkp,temp,i=0;
    FILE *fp;
    fp=fopen("reg.txt","r"); //File opened where the new password will be saved
    if(fp==NULL){
        printf("\nAn error occurred to open the file!\n");   //Checking whether the file is empty
        main();
    }
    printf("\n\t\tEnter username : "); //Details required to change password
    scanf(" %[^\n]",s);
    printf("\t\tEnter the current password : ");
    scanf(" %[^\n]",p);
    while(!feof(fp)){
        fread(&w[k], sizeof(struct Student), 1, fp);  //Reading the file details to check if the details entered are correct.
        chku=strcmp(s,w[k].name);  //Username and old password are being checked
        chkp=strcmp(p,w[k].pass);
        if(chku==0 && chkp==0){
            temp=1;
            break;
        }
    ...
    fclose(fp);
    fp=fopen("reg.txt","r+"); //Opening for reading and writing registration file
    if(fp==NULL){
        printf("\nAn error occurred to open the file!\n");   //Checking whether the file is empty
        main();
    }
        switch(temp){   //This flow of statement is used to check the valid details
        case 0:
            printf("\n\t\t< Wrong credentials! >\n");
            fclose(fp);
            AA:   //label for invalid details
             printf("\n\t\t*******************************************");
             printf("\n\t\tChoose from the MINI MENU below: ");
             printf("\n\t\t*******************************************");

            printf("\n\t\t 1. Try again\n\t\t 2. Go to menu\n");//Mini menu displayed for the user
            printf("\n\t\tEnter option : ");
            scanf("%d",&q);

            switch(q){
                case 1:
                    change_password(); //Calling the function again
                    break;
                case 2:
                    post_login();  //Calling the main menu of the system
                    break;
                default:
                    printf("\n\t\tInvalid input! Enter valid input.\n"); //Default messages for incorrect inputs
                    goto AA;
            break;
            }
        case 1:  //Case for entering new password
            printf("\n\t\tEnter new password:");
            scanf(" %[^\n]",p);
            strcpy(w[i].pass,p);
            fwrite(&w, sizeof(struct Student), 1, fp); //Writing the new password in the file
            printf("\n\t\t < Password Updated Successfully! >\n");
            fclose(fp); //Closing the file reg.txt
            break;
    }
    post_login(); //calling the main menu again once logged in
}
...
void list(){
    ...
    while(fread(&inf, sizeof(struct info), 1, fp)){ //reading the file from first of the file info.txt till end
        ...
        printf("\n\n\t\t%d.} Name : %s\n\t\t> Employee number : %s\n\t\t> Email : %s\n\t\t> Phone number : %s\n\t\t> Designation : %s\n\t\t> Salary(CTC) : %s\n\t\t",i+1,inf.name,inf.id,inf.email,inf.no,inf.desig,inf.sal);
...
void Delete(){
    ...
    while(another == 'y'){
        ...
        rename("temp.txt","info.txt"); // rename the temp file to original file name

        printf("\n\t\tWant to Delete another data (y\n)?");  // while loop will be executed if required
        scanf("%c",&another);
    }
    post_login();
}
...
void search(int i){
    ...
    while(fread(&in,sizeof(struct info),1,fp)){  // Read the record of file into.txt
        ...
        if(k==0){
            ...
            printf("\n\t\t> Name of employee : %s\n\t\t> Employee ID : %s\n\t\t> Email : %s\n\t\t> Contact number : %s\n\t\t> Designation : %s\n\t\t> Salary(CTC) : %s\n\t\t",in.name,in.id,in.email,in.no,in.desig,in.sal);
            break;
        }
    }
    fclose(fp);
